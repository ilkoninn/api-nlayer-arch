...
- Title: **Suntor API**
- Version: `v1`
- Description: **Suntoro Backend API**
...
### 5.1 Lokal DB vs host DB (sadə qayda)
ASP.NET birləşdirir: `appsettings.json` (ümumi — əsasən **Serilog**) + `appsettings.{Environment}.json`.

- **`appsettings.json`:** paylaşılan **Serilog** + `AllowedHosts`. DB/JWT/CORS/API URL burada **yoxdur** — mühit faylında.
- **`ASPNETCORE_ENVIRONMENT=Development` (lokal `dotnet run`):** `appsettings.Development.json` — lokal MySQL, lokal JWT, `localhost:7190` API, Vite CORS. Fayl `.gitignore`-dadır.
- **`ASPNETCORE_ENVIRONMENT=Production` (development hosting):** `appsettings.Production.json` — uzaq DB, host API URL, Vercel/host CORS. **Hostda** `Development` mühiti ilə köhnə tək `appsettings.json` trükü artıq kifayət etməyə bilər — **Production** mühit + `appsettings.Production.json` yüklənməlidir.

Şablonlar (köçürmə üçün): `appsettings.Development.template.json`, `appsettings.Production.template.json`. Ümumi nümunə: `appsettings.example.json`.
